...
- **ランダムキャラクター選出** : レポートを作成し、プロセカキャラクターの中からランダムに1人が選ばれ、指定ブランチにコミットしてくれます
- **依存関係のチェック** : `npm outdated`を実施し、古い依存関係一覧をレポートします
- **脆弱性のチェック** : `npm audit`を実施し、脆弱性をレポートします
- **レポート作成** : 作成したレポートは`NPM_SECURITY_REPORT.md`として指定ブランチにコミットします（コミットAuthor/Committerは選出されたプロセカキャラクターになります）
- **PR自動作成** : レポートをコミットした後、指定ブランチからデフォルトブランチへのPRを自動で作成します（既にオープンなPRがある場合はスキップ）
